...
# GGUF mode: AutoModel/AutoTokenizer replaced by llama-cpp-python
...
# ---- Q4_K_M (5.03GB): mixed-precision quant, attention/ff key layers use Q6_K ----
# ---- recommended for multimodal vision tasks (better quality) ----
GGUF_MODEL_FILE = "MiniCPM-o-4_5-Q4_K_M.gguf"
# ---- Q4_K_S (4.8GB): uniform Q4_K quant, saves ~230MB VRAM, slightly lower quality ----
# GGUF_MODEL_FILE = "MiniCPM-o-4_5-Q4_K_S.gguf"
GGUF_VISION_FILE = "vision/MiniCPM-o-4_5-vision-F16.gguf"
GGUF_N_CTX = 4096
...
class GGUFMiniCPMModel:
    """
    Wrapper around llama-cpp-python Llama for GGUF-based MiniCPM-o 4.5.
    Exposes .chat() compatible with HuggingFace AutoModel so that
    MiniCPMAgent / chat_step / chat_step_stream remain unchanged.
    """

    def __init__(self, model_path: str, clip_model_path: str = None,
    ...
    def eval(self):
        """No-op for compat with AutoModel().eval() chaining."""
        return self

    @staticmethod
    def _pil_to_data_uri(pil_img):
        """Convert PIL.Image to base64 data URI for OpenAI vision format."""
        buf = io.BytesIO()
    ...
    # ---- public interface (matches AutoModel.chat) ----

    def chat(self, msgs, tokenizer=None, stream=False,
             max_new_tokens=256, **kwargs):
        """
        Drop-in replacement for MiniCPM AutoModel.chat().

        Parameters
        ----------
        msgs          : MiniCPM format message list
        tokenizer     : Ignored (GGUF tokenizes internally)
        stream        : True  -> yields text delta chunks
                        False -> returns full response str
        max_new_tokens: max generation length
        **kwargs      : Ignored (enable_thinking, use_tts_template, etc.)
        """
        openai_msgs = self._convert_msgs(msgs)
        if stream:
            return self._stream_generate(openai_msgs, max_new_tokens)
    ...
    def _stream_generate(self, openai_msgs, max_tokens):
        """Yield text delta strings (not cumulative)."""
        resp = self.llm.create_chat_completion(
            messages=openai_msgs,
...
def get_minicpm_model():
    """Download (if needed) and load GGUF model + vision projector."""
    global _MODEL
    ...
    print(f"[MODEL] Downloading GGUF files from {GGUF_REPO_ID} ...")
    print(f"[MODEL]   LLM file:    {GGUF_MODEL_FILE}")
    print(f"[MODEL]   Vision file: {GGUF_VISION_FILE}")

    t0 = time.time()
    model_path = hf_hub_download(repo_id=GGUF_REPO_ID, filename=GGUF_MODEL_FILE)
    mmproj_path = hf_hub_download(repo_id=GGUF_REPO_ID, filename=GGUF_VISION_FILE)
    print(f"[MODEL] GGUF files ready (download/cache) in {time.time() - t0:.1f}s")

    _MODEL = GGUFMiniCPMModel(
        model_path=model_path,
...
def get_minicpm_tokenizer():
    """GGUF handles tokenization internally; return None placeholder."""
    global _TOKENIZER
    if _TOKENIZER is not None:
        return _TOKENIZER
    print("[MODEL] GGUF mode: tokenizer not needed (handled by llama.cpp)")
    _TOKENIZER = None
    return _TOKENIZER
...
# Gradio UI
# -----------------------------
with gr.Blocks(title="MiniCPM-o-4.5 GGUF Multimodal Chatbot (Q4_K_M)") as demo:
    gr.Markdown(
        """
# MiniCPM-o-4.5 GGUF Multimodal Chatbot (Q4_K_M)
